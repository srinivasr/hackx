...
def compute_model_stability(p_res: Dict[str, Any], base_res: Dict[str, Any]) -> float:
    """Computes dynamic retained model stability percentage under stress perturbation.
    Measures retention of baseline diagnostic decision and confidence."""
    base_grade = base_res.get("predicted_grade", 0)
    base_conf = max(5.0, float(base_res.get("confidence", 50.0)))
    p_grade = p_res.get("predicted_grade", 0)
    p_conf = float(p_res.get("confidence", 50.0))

    if p_grade == base_grade:
        # Diagnosis preserved: stability is proportional to confidence preservation
        retention = (p_conf / base_conf) * 100.0
        return round(float(np.clip(retention, 15.0, 100.0)), 1)
    else:
        # Diagnostic grade flipped: clinical failure under stress
        grade_dist = abs(p_grade - base_grade)
        decay = 1.0 / (1.0 + grade_dist * 0.8)
        retention = (p_conf / 100.0) * decay * 42.0
        return round(float(np.clip(retention, 5.0, 48.0)), 1)
...
def run_full_model_audit(
    ...
    base_res = baseline_predictions.get(ref_key, evaluator.infer(ref_img))
